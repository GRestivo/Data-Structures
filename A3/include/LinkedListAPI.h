...
/*typedef struct sListNode {
  void *data;
  struct sListNode *previous;
  struct sListNode *next;
} sNode;
*/
/*
typedef struct secondListHead {
  sNode *head;
  sNode *tail;
...
/*sList *intializeSList(void (*printFunction)(void *toBePrinted), void (*deleteFunction)(void *toBeDeleted));*/
...
/*sNode *intializeSNode(List *slist, void *data);*/
...
/*Already tested, just try different values*/
...
/**Function to remove a node from the front of a list and alter pointers accordingly
 *@pre List must exist to have memory allocated to it
 *@post The node at the front of the list will be freed
 *@param List pointer to the list from which the first node will be deleted 
 *@return None
 **/
...
/*point other head to head->next->next
point to be deleted to ->NULL
same problem,try it*/
